...
import { fetchMoveArticle, fetchGetCategory } from '@/api/article';

export default function MoveArticle(props: ModalProp.OperateModalProp<number[]>) {
  const [form] = useForm();
  ...
  return (
    <Modal
      title={action === 'create' ? '移动文章到分类' : '移动文章到分类'}
      width={800}
      open={visible}
      okText="确定"
      cancelText="取消"
