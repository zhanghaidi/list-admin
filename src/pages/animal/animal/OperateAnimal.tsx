...
import { fetchCreateAnimal, fetchUpdateAnimal } from '@/api/animal';
import { fetchGetCategory } from '@/api/category';
import { fetchUpload } from '@/api/upload';
import { getImageUrl } from '@/utils';

export default function OperateAnimal(props: ModalProp.OperateModalProp<Api.AnimalManger.Animal>) {
  ...
  const [categories, setCategories] = useState<Api.AnimalManger.AnimalCategory[]>([]);
  ...
  const handleSubmit = async () => {
    ...
    if (valid) {
      ...
      if (action === 'create') {
        await fetchCreateAnimal(params);
      } else {
        await fetchUpdateAnimal(params);
      }
    ...
    >
      <Form className="modal-from" form={form} labelAlign="right" labelCol={{ span: 4 }}>
        ...
        <Form.Item label="所属分类" name="categoryId" rules={[{ required: true, message: '请选择模型所属分类' }]}>
          <Select style={{ width: 200 }} placeholder="请选择所属分类" allowClear>
            {categories.map((item) => {
              return (
